...
public abstract class BaseForm extends BaseEntity {
	...
	/**
	 * Найти все элементы на странице по Xpath
	 * @param xpathString путь
	 * @return лист элементов
	 */
	public  List<WebElement> findElementsByXpath(String xpathString){
		List<WebElement> resultsList = browser.getDriver().findElements(By.xpath(xpathString));
		return resultsList;
	}
	...
	public void switchToModalFrame(String className) {
		WebElement modalFrame = browser.getDriver().findElement(By.className(className));
		browser.getDriver().switchTo().frame(modalFrame);
	}

	/**
	 * Переключиться на дефолт контент
	 */
