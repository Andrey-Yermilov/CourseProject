...
public class ItemForm extends BaseForm {
    ...
    private Label lblFullItemName = new Label(By.xpath("//h2[@class='catalog-masthead__title']"),"Full item name");
    private  Label lblArrangeByPrice = new Label(By.xpath("//div[@class='b-offers-list-line-1__group b-offers-sort']//a[contains(text(),'По цене')]"), "By price");
    ...
    private int countOffers(String city){
        String pathTemplate = "//tr[contains(@class, 'js-position-item ') and contains(@id, '%s')]";
        return  findElementsByXpath(String.format(pathTemplate,city)).size();
    }
    ...
    private int getItemPrice(int i, String city){
        String itemPath = String.format("(//tr[contains(@class, 'js-position-item ') and contains(@id, '%s')]//a[@class='js-currency-primary'])", city)+"[%d]";
